      ...
      <div className="card relative my-auto w-full max-w-2xl p-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <input type="number" step="0.1" min="0" max="10" {...register('rating')} className="input"/>
          </div>
          <div>
    ...
    return (<div className="fixed inset-0 bg-dark-900/80 z-50 flex items-center justify-center">
      <div className="card p-6 w-full max-w-md">
        ...
        <div className="flex justify-end space-x-4">
          ...
          <button onClick={() => onConfirm(movie._id)} className="btn btn-danger">
            Delete Movie
          </button>
        </div>
      </div>
...
export default function AdminMoviesPage() {
    ...
    return (<div className="space-y-6">
      {isModalOpen && (<MovieForm movieToEdit={editingMovie} onClose={handleCloseModal} onSave={handleSave}/>)}
      {movieToDelete && (<DeleteConfirmationModal movie={movieToDelete} onClose={() => setMovieToDelete(null)} onConfirm={handleConfirmDelete}/>)}
      ...
      <div className="cinema-panel grid grid-cols-1 gap-4 p-4 md:grid-cols-3">
        <input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} className="input" placeholder="Search movies..."/>
      ...
      {filteredMovies.length === 0 ? (<div className="text-center py-12 card">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-dark-700">
                {filteredMovies.map((movie) => (<tr key={movie._id} className="hover:bg-dark-800/50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <img src={movie.poster_url || 'https://placehold.co/100x150/0f172a/94a3b8?text=No+Image'} alt={movie.title} className="w-12 h-18 object-cover rounded"/>
                        <div className="ml-4">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center space-x-2">
                        ...
                        <button onClick={() => handleDeleteClick(movie)} className="text-red-400 hover:text-red-300 p-2 rounded-full hover:bg-dark-700">
