...
export default function MovieDetailsPage() {
  ...
  useEffect(() => {
    const loadFilters = async () => {
      try {
        const [locationsData, bioskopsData] = await Promise.all([locationService.getLocations(), bioskopService.getBioskops()]);
  ...
  const fetchMovie = async (movieId) => {
    try {
      const [movieData, showtimeData] = await Promise.all([movieService.getMovieById(movieId), showtimeService.getMovieShowtimes(movieId)]);
  ...
  const filteredBioskops = useMemo(() => bioskops.filter((bioskop) => !selectedLocation || getId(bioskop.locationId) === selectedLocation), [bioskops, selectedLocation]);
  ...
  const availableDates = useMemo(() => [...new Set(showtimesForCinema.map((showtime) => showtime.show_date.split('T')[0]))], [showtimesForCinema]);
            ...
            <div className="max-w-3xl space-y-6">
              <div>
                ...
                <div className="flex flex-wrap items-center gap-6 text-slate-400 mb-6">
                  ...
                  <div className="px-3 py-1 bg-primary-500/20 text-primary-400 rounded-full text-sm font-medium">{Array.isArray(movie.genre) ? movie.genre.join(', ') : movie.genre}</div>
                </div>
              </div>

              <div className="flex flex-col gap-3 sm:flex-row">
                {canBook ? (
                  <Link to={selectedShowtime ? `/booking/${selectedShowtime._id}` : `/book/${movie._id}`} className="btn btn-primary text-lg px-8 py-3">
                    <Ticket className="h-5 w-5" />
                    Buy Tickets
                  </Link>
                ) : (
                  <button type="button" className="btn btn-secondary text-lg px-8 py-3" disabled>
                    <Ticket className="h-5 w-5" />
                    No showtime yet
                  </button>
                )}
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-slate-300">{movie.cast && movie.cast.length > 0 ? movie.cast.join(', ') : 'To be announced'}</p>
              </div>
            </div>
            ...
            {showtimes.length > 0 ? (
              <div className="cinema-panel sticky top-24 p-6">
                ...
                <div className="mb-5 grid gap-3">
                  ...
                  <div>
                    ...
                    >
                      ...
                      {filteredBioskops.map((bioskop) => (
                        <option key={bioskop._id} value={bioskop._id}>
                          {bioskop.name}
                        </option>
                      ))}
                    </select>
                ...
                <div className="mb-3 flex items-center gap-2 text-sm text-slate-400">
                  ...
                  Pilih lokasi, bioskop, dan tanggal tayang.
                </div>
                <div className="mb-5 flex gap-2 overflow-x-auto pb-1">
                  {availableDates.map((date) => (
                    ...
                    >
                      <span className="block text-xs uppercase opacity-70">{new Date(date).toLocaleDateString('en-US', { weekday: 'short' })}</span>
                      <span className="block font-bold">{new Date(date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                    </button>
                  ))}
                </div>
                  ...
                  {visibleShowtimes.length > 0 ? (
                    visibleShowtimes.map((showtime) => (
                      <button
                        ...
                        className={`w-full rounded-lg border p-4 text-left transition hover:border-primary-500 ${selectedShowtime?._id === showtime._id ? 'border-primary-500 bg-primary-500/10' : ''}`}
                      >
                        ...
                        <p className="text-sm text-slate-400">
                          {showtime.hall.hall_name} • {showtime.end_time} finish
                        </p>
                      </button>
                    ))
                  ) : (
                    <p className="rounded-lg border border-dashed border-white/15 p-4 text-sm text-slate-400">Tidak ada jadwal untuk filter yang dipilih.</p>
                  )}
                </div>
