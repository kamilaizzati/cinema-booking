...
import { LayoutDashboard, Film, Building, Calendar, Users, LogOut, Menu, Moon, Sun, X, BarChart3, MapPin, Armchair, Receipt } from 'lucide-react';
...
export default function AdminLayout() {
    ...
    return (<div className="admin-shell min-h-screen bg-dark-950">
      {/* Mobile sidebar */}
      <div className={`fixed inset-0 z-50 lg:hidden ${sidebarOpen ? 'block' : 'hidden'}`}>
        ...
        <div className="admin-sidebar fixed inset-y-0 left-0 w-64 bg-dark-900 border-r border-dark-700">
          ...
          <nav className="mt-8">
            ...
            const isActive = location.pathname === item.href || (item.href === '/admin/studios' && location.pathname === '/admin/halls');
            return (<Link key={item.name} to={item.href} onClick={() => setSidebarOpen(false)} className={`flex items-center px-4 py-3 text-sm font-medium transition-colors ${isActive
      ...
      <div className="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-64 lg:flex-col">
        <div className="admin-sidebar flex flex-col flex-grow bg-dark-900 border-r border-dark-700">
          <div className="flex items-center h-16 px-4 border-b border-dark-700">
            <h2 className="text-xl font-display font-black uppercase tracking-tight text-primary-500">Cinema<span className="text-white">Ops</span></h2>
          </div>
          <nav className="mt-8 flex-1">
            {navigation.map((item) => {
            const Icon = item.icon;
            const isActive = location.pathname === item.href || (item.href === '/admin/studios' && location.pathname === '/admin/halls');
            return (<Link key={item.name} to={item.href} className={`flex items-center px-4 py-3 text-sm font-medium transition-colors ${isActive
                    ? 'bg-primary-500/20 text-primary-400 border-r-2 border-primary-500'
                    : 'text-slate-300 hover:text-white hover:bg-dark-800'}`}>
                  <Icon className="mr-3 h-5 w-5"/>
                  {item.name}
          ...
          <div className="p-4 border-t border-dark-700">
            ...
            <button onClick={handleSignOut} className="flex items-center w-full px-3 py-2 text-sm text-slate-300 hover:text-white hover:bg-dark-800 rounded-lg transition-colors">
