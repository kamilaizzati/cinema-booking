    ...
    return (<div className="movie-card group flex h-full flex-col">
      <div className="relative aspect-[2/3] flex-none overflow-hidden">
        ...
        <div className="absolute left-3 right-3 top-3 flex items-start justify-between gap-2">
          ...
          {movie.classification && (<span className="rounded bg-dark-950/80 px-2 py-1 text-xs font-bold text-white">
              {movie.classification}
            </span>)}
        </div>
        {!isComingSoon && movie.rating ? (<div className="absolute bottom-3 left-3 flex items-center gap-1 rounded-full bg-dark-950/85 px-3 py-1 text-sm font-semibold text-accent-300">
        ...
        <div className="absolute inset-x-3 bottom-3 hidden translate-y-3 flex-col gap-2 opacity-0 transition-all duration-300 group-hover:flex group-hover:translate-y-0 group-hover:opacity-100">
          ...
          <Link to={canBook ? `/book/${movie._id}` : `/movies/${movie._id}`} className={`btn w-full ${canBook ? 'btn-primary' : 'btn-secondary'}`}>
            {canBook ? <Ticket className="h-4 w-4"/> : <Play className="h-4 w-4"/>}
      ...
      <div className="flex min-h-[210px] flex-1 flex-col p-4">
        ...
        <div className="mt-auto flex min-h-9 items-center justify-between gap-3 border-t border-white/10 pt-3 text-xs font-semibold uppercase tracking-wide text-slate-400">
            <span className="min-w-0 truncate">{canBook ? 'Tickets Open' : 'No showtime yet'}</span>
            <Link to={`/movies/${movie._id}`} className="shrink-0 text-primary-400 transition hover:text-primary-300">
              Open
            </Link>
        </div>
      </div>
