...
export const showtimeService = {
  ...
  async updateHall(id, hallData) {
    ...
    if (!data.success) throw new Error(data.message || 'Gagal mengupdate studio');
    return { ...data.data, hall_name: data.data.name };
  },

  async deleteHall(id) {
    const { data } = await api.delete(`/studios/${id}`);
    if (!data.success) throw new Error(data.message || 'Gagal menghapus studio');
  },
  ...
  async createShowtime(showtimeData) {
    ...
    const payload = {
      ...
      studio: showtimeData.hallName || showtimeData.hall?.hall_name || showtimeData.studio,
      price: showtimeData.ticket_price || showtimeData.price,
    };
    const { data } = await api.post('/showtimes/showtimes', payload);
    if (!data.success) throw new Error(data.message || 'Gagal membuat showtime');
    return normalizeShowtime(data.data);
  },
  ...
  async updateShowtime(id, showtimeData) {
    ...
    if (!data.success) throw new Error(data.message || 'Gagal mengupdate showtime');
    return normalizeShowtime(data.data);
  },

  async deleteShowtime(id) {
    const { data } = await api.delete(`/showtimes/showtimes/${id}`);
    if (!data.success) throw new Error(data.message || 'Gagal menghapus showtime');
  },
};
