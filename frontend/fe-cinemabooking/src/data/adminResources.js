...
export const transactionConfig = {
  ...
  columns: [{ name: 'bookingId._id', label: 'Booking', render: (item) => `#${(item.bookingId?._id || item.bookingId || '').slice(-6)}` }, { name: 'userId.name', label: 'User' }, { name: 'amount', label: 'Jumlah', render: (item) => `IDR ${item.amount.toLocaleString()}` }, { name: 'paymentMethod', label: 'Metode' }, { name: 'status', label: 'Status' }],
};
